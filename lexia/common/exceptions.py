...
Comprehensive exception hierarchy for Lexia SDK.
...
class LexiaException(Exception):
    """
    Base exception for all Lexia SDK exceptions.
    
    All custom exceptions should inherit from this base class.
    Allows users to catch all Lexia-related errors with a single except clause.
...
class ConfigurationError(LexiaException):
...
class ValidationError(LexiaException):
...
class CommunicationError(LexiaException):
...
class BufferError(LexiaException):
...
class UsageTrackingError(LexiaException):
...
def wrap_exception(
    exception: Exception,
    message: str,
    exception_class: type = LexiaException
) -> LexiaException:
    """
    Wrap an exception in a Lexia exception.
