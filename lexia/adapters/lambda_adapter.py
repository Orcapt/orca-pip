...
Adapter برای اجرای راحت Lexia agents روی AWS Lambda.
...
from lexia.domain.models import ChatMessage
...
class LambdaAdapter:
    ...
    Example:
        >>> from lexia import LambdaAdapter, LexiaHandler
        >>> 
...
def lambda_handler(func: Callable = None, *, dev_mode: bool = False, enable_sqs: bool = True, enable_cron: bool = True):
    """
    Ultimate simplicity decorator - یک خطی!
    
    این decorator همه setup رو خودش انجام میده:
    - LexiaHandler
    - LambdaAdapter
    - Session management
    - Error handling
    ...
    Example (simplest possible):
        >>> from lexia import lambda_handler
        >>> 
        >>> @lambda_handler
        >>> async def my_agent(session, data):
        ...     session.stream(f"You said: {data.message}")
        ...     session.close()
    
    Example (با options):
        >>> @lambda_handler(dev_mode=True, enable_sqs=False)
        >>> async def my_agent(session, data):
        ...     session.stream("Response")
        ...     session.close()
    
    Args:
        func: Agent function که (session, data) میگیره
        dev_mode: Development mode (default: False)
        enable_sqs: فعال‌سازی SQS (default: True)
        enable_cron: فعال‌سازی cron (default: True)
    
    Returns:
        Lambda handler function (برای AWS Lambda)
    """
    def decorator(agent_func: Callable):
        # Import در runtime برای circular import جلوگیری
        from lexia.core.handler import LexiaHandler
        
        handler = LexiaHandler(dev_mode=dev_mode)
        
        @wraps(agent_func)
        async def message_handler(data: ChatMessage):
            session = handler.begin(data)
            try:
                await agent_func(session, data)
            except Exception as e:
    ...
    # Support both @lambda_handler and @lambda_handler()
    if func is None:
        return decorator
    else:
...
__all__ = ['LambdaAdapter', 'create_lambda_handler', 'lambda_handler']
