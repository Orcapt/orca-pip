# Orca SDK Documentation

Complete documentation for the Orca SDK.

## Table of Contents

### Getting Started

- [Quick Reference](guides/QUICK_REFERENCE.md) - API reference and examples
- [Developer Guide](guides/DEVELOPER_GUIDE.md) - Complete development guide
- [Dev Mode Guide](guides/DEV_MODE_GUIDE.md) - Local development setup

### Core Documentation

- [API Reference](guides/API_REFERENCE.md) - Complete API documentation
- [Utilities Guide](guides/UTILITIES_GUIDE.md) - Exceptions, decorators, logging

### Advanced Features

- [Patterns Guide](guides/PATTERNS_GUIDE.md) - Design patterns (Builder, Middleware, Context)

### Deployment

- [Lambda Deploy Guide](guides/LAMBDA_DEPLOY_GUIDE.md) - AWS Lambda deployment
- [Storage SDK Guide](guides/STORAGE_SDK_GUIDE.md) - Storage SDK and API reference

## Quick Links

- [Main README](../README.md)
- [Examples Directory](../examples/)
- [Contributing Guidelines](../CONTRIBUTING.md)
- [Security Policy](../SECURITY.md)
- [Changelog](../CHANGELOG.md)

## Architecture

Orca SDK follows clean architecture with these layers:

1. **Core** - Handler and session management
2. **Domain** - Interfaces and models
3. **Services** - Business logic
4. **Infrastructure** - External clients
5. **Factories** - Object creation
6. **Helpers** - Utility helpers
7. **Utils** - General utilities
8. **Common** - Cross-cutting concerns
9. **Patterns** - Design patterns
10. **Adapters** - Deployment adapters
11. **Storage** - File storage management

## Support

For questions or issues:

- Email: [support@orcapt.com](mailto:support@orcapt.com)
- Discord: [Join our community](https://discord.gg/orca)
- GitHub: [orcapt/orca-pip](https://github.com/orcapt/orca-pip/issues)